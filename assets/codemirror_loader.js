...
const loadCodeMirror = (id, height, mode, onchange, extraKeys, readOnly = false) => document.addEventListener('DOMContentLoaded', () => {
    const textarea = document.getElementById(id);
    const readOnlyBool = readOnly === true || readOnly === 'true';
    ...
    let timer = null;
    editor.on('change', () => {
        if (readOnlyBool) return;
        editor.save();
        clearTimeout(timer);
        timer = setTimeout(onchange, 250);
    });
    editor.setSize(null, height);
    if (!readOnlyBool) onchange();
