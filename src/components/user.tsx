...
export const userQuery = async (uid: number, c: ContextType): Promise<userInfo | null> =>
    await (c.env as any).db.prepare('SELECT id, name, created_at, permission, username_violation, name_color_light, name_color_dark FROM users WHERE id = ?').bind(uid).first();
...
export const User: FC<{
    user: userInfo | number | null,
    c: ContextType,
    hideTag?: boolean
}> = async ({ user, c, hideTag = false }) => {
    const locale = c.get('locale');
    if (user === null) return <span>{getText(locale, 'notLoggedIn')}</span>;
    if (user === undefined) return <span>{getText(locale, 'userUnknown')}</span>;
    if (typeof user === 'number') {
        if (!user) return <span>{getText(locale, 'userUnknown')}</span>;
        const resolvedUser = await userQuery(user, c);
        return resolvedUser ? <User user={resolvedUser} c={c} hideTag={hideTag} /> : <span>{getText(locale, 'userUnknown')}</span>;
    }
    const tag = (!hideTag && (user.permission & permissionAdmin)) ? getText(locale, 'userTagAdmin') : null;
