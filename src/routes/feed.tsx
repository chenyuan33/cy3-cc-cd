...
app.get('/:feed_id{[1-9][0-9]*}?', async c => {
	const feed_id = parseInt(c.req.param('feed_id') || '0'), env = c.env as any, locale = c.get('locale'), translations = c.get('translations'), currentUser = c.get('currentUser');
		...
		<Card>
			...
			{feed_id ? <Card>
				...
				{currentUser && (currentUser.id === 1 || currentUser.id === uid) ? <>
					<button type='button' onclick={`document.getElementById('feed-edit-${feed_id}').dataset.vis *= -1`}>{translations.edit}</button>
					&nbsp;
